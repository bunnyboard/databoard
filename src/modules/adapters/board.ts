...
// but data will be saved to boards collections
export default class BoardAdapter extends ProtocolAdapter implements IProtocolAdapter {
  ...
  public async run(options: RunAdapterOptions): Promise<void> {
    ...
    if (!options.service || options.service === 'state') {
      this.executeSession.startSession('start to update board current data', {
        service: this.name,
      ...
      if (last24HoursData) {
        if (last48HoursData) {
          await this.storages.database.update({
            collection: envConfig.mongodb.collections.boardDataStates.name,
            keys: {
      ...
      this.executeSession.endSession('updated current data for board', {
        service: this.name,
        protocol: this.protocolConfig.protocol,
      });
    }
    ...
    if (!options.service || options.service === 'snapshot') {
      ...
      if (!options.force) {
        ...
        const latestSnapshot = (
          await this.storages.database.query({
            collection: envConfig.mongodb.collections.boardDataSnapshots.name,
            query: {
      ...
      logger.info('start to update board snapshots data', {
        service: this.name,
        chain: this.protocolConfig.protocol,
      ...
      while (startTime <= todayTimestamp) {
        this.executeSession.startSession('start to update board snapshot', {
          service: this.name,
          chain: this.protocolConfig.protocol,
        ...
        this.executeSession.endSession('updated snapshot data for board', {
          service: this.name,
          chain: this.protocolConfig.protocol,
          date: getDateString(startTime),
        });
