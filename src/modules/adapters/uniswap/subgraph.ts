...
export async function querySubgraphUniv2(options: SubgraphQueryOptions): Promise<SubgraphQueryResult | null> {
        ...
        for (const event of mintEvents.concat(burnEvents)) {
          if (event.amount0 && event.amount1) {
            ...
            if (TokenDexBase[factoryConfig.chain] && TokenDexBase[factoryConfig.chain].includes(token0Address)) {
              const tokenPriceUsd = await options.contextServices.oracle.getTokenPriceUsdRounded({
...
export async function querySubgraphUniv3(options: SubgraphQueryOptions): Promise<SubgraphQueryResult | null> {
  const { factoryConfig, params, blockNumber, fromBlock, timestamp, fromTime, toTime } = options;
  let { toBlock } = options;
      ...
      if (fromFactoryData && toFactoryData) {
        ...
        if (options.countPools) {
          ...
          do {
            ...
            const balances = await options.contextServices.blockchain.evm.multicall({
              chain: factoryConfig.chain,
            ...
            if (balances) {
              for (let i = 0; i < pools.length; i++) {
                ...
                if (TokenDexBase[factoryConfig.chain] && TokenDexBase[factoryConfig.chain].includes(token0Address)) {
                  const token0PriceUsd = await options.contextServices.oracle.getTokenPriceUsdRounded({
                    chain: factoryConfig.chain,
                    address: token0Address,
                    timestamp: timestamp,
                  });
                  ...
                  const balance0Usd =
                    formatBigNumberToNumber(balances[i * 2].toString(), Number(pool.token0.decimals)) * token0PriceUsd;
                  ...
                  const token1PriceUsd = await options.contextServices.oracle.getTokenPriceUsdRounded({
                    chain: factoryConfig.chain,
                    address: token1Address,
                    timestamp: timestamp,
                  });
