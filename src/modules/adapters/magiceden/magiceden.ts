    ...
    for (const processorConfig of magicedenConfig.paymentProcessors) {
      ...
      for (const log of logs.filter((log) => Object.values(Events).includes(log.topics[0]))) {
        ...
        if (token) {
          ...
          (protocolData.volumeMarketplace as any).sale += purchaseAmountUsd;
          ...
          (protocolData.breakdown[token.chain][token.address].volumeMarketplace as any).sale += purchaseAmountUsd;
        }
      }
    }
