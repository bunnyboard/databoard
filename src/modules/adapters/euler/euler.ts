...
export default class EulerAdapter extends ProtocolAdapter {
  ...
  public async getProtocolData(options: GetProtocolDataOptions): Promise<ProtocolData | null> {
    ...
    if (blockNumber <= anchorBlock) {
      ...
      // get interest rates
      // before - beginBlock
      const before_getInterestAccumulatorCalls: Array<ContractCall> = eulerConfig.v1Config.tokens.map(
        (tokenAddress) => {
          return {
            abi: EulerMarketsAbi,
            target: eulerConfig.v1Config.marketProxy,
    ...
    for (const factoryConfig of eulerConfig.factories) {
      ...
      const vaultProxyList: Array<string> = await this.services.blockchain.evm.multicall({
        chain: factoryConfig.chain,
        blockNumber: blockNumber,
