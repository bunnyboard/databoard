    ...
    for (const log of logs.filter((log) => Object.values(Events).includes(log.topics[0]))) {
      ...
      if (token) {
        ...
        if (protocolData.breakdownCollectibles && amountUsd > 0) {
          const collection = normalizeAddress(event.args._originContract);
          if (!protocolData.breakdownCollectibles[superrareConfig.chain][collection]) {
            protocolData.breakdownCollectibles[superrareConfig.chain][collection] = {
              volumeTrade: 0,
              totalFees: 0,
              protocolFee: 0,
              royaltyFee: 0,
          ...
          protocolData.breakdownCollectibles[superrareConfig.chain][collection].volumeTrade += amountUsd;
          protocolData.breakdownCollectibles[superrareConfig.chain][collection].totalFees += feeUsd;
          protocolData.breakdownCollectibles[superrareConfig.chain][collection].protocolFee += feeUsd;
        }
      }
    }
